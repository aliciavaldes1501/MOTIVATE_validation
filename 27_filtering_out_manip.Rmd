---
title: "Script to exclude duplicates and filter out manipulated plots from ReSurvey database v2025.2"
...
This R script is used to exclude duplicates and filter out manipulated plots from ReSurvey database v2025.2, starting from the raw data downloaded from:
...
```{r}
nrow(data_V2025.2 %>% filter(!is.na(RS_CODE))) 
# Includes observations in ReSurvey DB
nrow(data_V2025.2 %>% filter(`ReSurvey plot (Y/N)` == "Y"))
...
ggplot(data_V2025.2 %>% filter(!is.na(EUNISa_1)),
       aes(EUNISa_1)) +
         geom_bar(aes(y = (..count..) / sum(..count..) * 100)) +
  labs(y = "Percentage of ReSurvey observations",
       x = "EUNIS level 1") + coord_flip()
...
```{r}
data_V2025.2 %>% 
  dplyr::filter(EUNISa_1 %in% c("Q", "R", "S", "T")) %>% nrow()
```
...
BUT there are cases with different coordinates wihin the same plot, not at all sure about this number, ask Illona. 
...
Cases where the expert system assigned more than one EUNIS habitat type at level 1:

```{r}
data_V2025.2 %>% 
  dplyr::filter(EUNISa_1 %in% c("Q", "R", "S", "T")) %>% 
  filter(!is.na(EUNISb_1) & EUNISa_1 != EUNISb_1) 
```

# Save list of PlotObservationIDs to filter other datasets
...
write_csv(data_V2025.2 %>%
          ...
          here("data", "clean","PlotObservationIDs_filtered_20260122.csv"))
```
