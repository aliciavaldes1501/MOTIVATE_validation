...
These files contain all observations in the ReSurvey database from 2016 onward. In order to avoid computation problems in GEE, biogeographical units that contain more than 4500 points have been subdivided in ArcGIS. Now I also divided files by years

```{r message=FALSE}
# Set the folder path
folder_path <- "C:/Data/MOTIVATE/MOTIVATE_RS_data/validation/S2/Years"
...
```{r}
data_RS_S2_bands <- data_RS_S2_bands %>%
  select(-UNIT) %>%
...
```{r}
data_RS_S2_bands_out <- read_csv(
  "C:/Data/MOTIVATE/MOTIVATE_RS_data/validation/S2_out/AllPoints_S2_Annual.csv") %>%
  rename(biogeo = BIOGEO, unit = UNIT,
         PlotObservationID = PltObID) %>%
  select(`system:index`, B11, B2, B3, B4, B8, PlotObservationID, date, image_id,
         time_utc ,timestamp, year, .geo, biogeo, unit)
```

Change unit names:

```{r}
data_RS_S2_bands_out <- data_RS_S2_bands_out %>%
  mutate(unit = case_when(
    unit == "ALPALPS" ~ "ALPS",
    unit == "ATLBENELUX" ~ "BENELUX",
    unit == "ATLBRITAIN" ~ "BRITAIN",
    unit == "CONAUSTRIA" ~ "AUSTRIA",
    unit == "CONGERMANY" ~ "GERMANY",
    unit == "CONITALICA" ~ "ITALICA",
    unit == "CONNORDIC" ~ "NORDIC",
    unit == "MEDITALICA" ~ "ITALICA",
    TRUE ~ unit
  ))
```
...
```{r}
# Columns in data_RS_S2_bands but not in data_RS_S2_bands_out
setdiff(names(data_RS_S2_bands), names(data_RS_S2_bands_out))

# Columns in data_RS_S2_bands_out but not in data_RS_S2_bands
setdiff(names(data_RS_S2_bands_out), names(data_RS_S2_bands))
```

# Join all data

```{r}
data_RS_S2_bands <- bind_rows(data_RS_S2_bands, data_RS_S2_bands_out)
```
...
data_RS_S2_bands_indices <- data_RS_S2_bands_updated %>%
  ...
  mutate(EVI = if_else(EVI > 1 | EVI < -1, NA, EVI)) # Some values of EVI as NA
```
...
GAM_data <- map_dfr(seq_along(batches), function(i) {
  ...
  result <- data_RS_S2_bands_indices %>%
    dplyr::filter(PlotObservationID %in% batch_ids) %>%
...
```{r}
GAM_data %>% 
  select(PlotObservationID, index, sos_slope, pos, eos_slope) %>% distinct() %>%
...
smoothed_data <- map_dfr(seq_along(batches), function(i) {
  batch_ids <- batches[[i]]
  total_batches <- length(batches)
  batch_file <- file.path("objects/smoothing_batches_S2", paste0("batch_", i, ".rds"))
...
```{r}
# Prepare the data
data_monthly_EUNIS1 <- data_RS_S2_bands_indices %>%
...
data_monthly_EUNIS2 <- data_RS_S2_bands_indices %>%
  mutate(month = month(date, label = TRUE, abbr = TRUE, locale="EN-us")) %>%
...
write_tsv(final_RS_data,
          here("data", "clean","final_RS_data_bands_S2_all_20260310.csv"))
```
