...
```{r}
# Set the folder path
folder_path <- "C:/Data/MOTIVATE/MOTIVATE_RS_data/S2/Bands"
...
```{r}
data_RS_S2_bands %>% filter(year != year(date))
```
...
db_Europa_sample_latest <- read_csv(
  ...
  select(PlotObservationID, EUNISa_1) %>%
  mutate(PlotObservationID = factor(PlotObservationID),
...
nrow(final_RS_data %>% filter(NDVI_gsd <= 0 | EVI_gsd <= 0))
# Difference in value between pos and sos should be positive
nrow(final_RS_data %>%
       filter(NDVI_diff_pos_sos_value <= 0))
nrow(final_RS_data %>%
       filter(EVI_diff_pos_sos_value <= 0))
# Difference in value between pos and eos should be positive
nrow(final_RS_data %>%
       filter(NDVI_diff_pos_eos_value <= 0))
nrow(final_RS_data %>%
       filter(EVI_diff_pos_eos_value <= 0))
# Difference in doy between pos and sos should be positive
nrow(final_RS_data %>%
       filter(NDVI_diff_pos_sos_doy <= 0 | EVI_diff_pos_sos_doy <= 0))
# Difference in doy between eos and pos should be positive
nrow(final_RS_data %>%
       filter(NDVI_diff_eos_pos_doy <= 0 | EVI_diff_eos_pos_doy <= 0))
```
...
plots_EVI_0peaks <- map(unique_ids[1:50], function(id) {
  plot_data <- GAM_data %>%
    filter(as.character(PlotObservationID) == id) %>%
    filter(fit_type == "observed" | fit_type == "iter_3")
...
```{r}
write_tsv(final_RS_data, here("data", "clean","final_RS_data_bands_S2.csv"))
```
