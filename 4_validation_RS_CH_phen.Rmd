...
db_resurv_RS<-read_tsv(
  here("data", "clean", "db_resurv_RS_20250331.csv"),
  col_types = cols(
...
db_resurv_RS_short <- db_resurv_RS %>%
  select(PlotObservationID, Country, RS_CODE, `ReSurvey site`, `ReSurvey plot`,
         ...
         canopy_height, SOS_DOY, Peak_DOY, EOS_DOY, S2_data, Landsat_data, CH_data, S2_phen_data)
```
...
ggplot(data = db_resurv_RS_short %>%
         # Keep only forests, grasslands, shrublands and wetlands
         filter(EUNISa_1 %in% c("T", "R", "S", "Q") & S2_phen_data == T),
       aes(x = EUNISa_1_descr, y = SOS_DOY, fill = EUNISa_1_descr)) +
  geom_flat_violin(position = position_nudge(x = 0.2, y = 0), alpha = 0.8) +
  geom_point(aes(y = SOS_DOY, color = EUNISa_1_descr),
             position = position_jitter(width = 0.15), size = 1, alpha = 0.25) +
  geom_boxplot(width = 0.2, outlier.shape = NA, alpha = 0.5) +
  stat_summary(fun.y=mean, geom="point", shape = 20, size = 1) +
  stat_summary(fun.data = function(x) data.frame(y = max(x) + 0.1,
                                                 label = length(x)),
               geom = "text", aes(label = ..label..), vjust = 0.5) +
  labs(y = "SOS_DOY", x = "EUNISa_1_descr") +
  guides(fill = FALSE, color = FALSE) +
  theme_bw() + coord_flip()
ggplot(data = db_resurv_RS_short %>%
         # Keep only forests, grasslands, shrublands and wetlands
  ...
  stat_summary(fun.data = function(x) data.frame(y = max(x) + 0.1,
                                                 label = length(x)),
               geom = "text", aes(label = ..label..), vjust = 0.5) +
  labs(y = "Peak_DOY", x = "EUNISa_1_descr") +
  guides(fill = FALSE, color = FALSE) +
  theme_bw() + coord_flip()
ggplot(data = db_resurv_RS_short %>%
         # Keep only forests, grasslands, shrublands and wetlands
         filter(EUNISa_1 %in% c("T", "R", "S", "Q") & S2_phen_data == T),
       aes(x = EUNISa_1_descr, y = EOS_DOY, fill = EUNISa_1_descr)) +
  geom_flat_violin(position = position_nudge(x = 0.2, y = 0), alpha = 0.8) +
  geom_point(aes(y = EOS_DOY, color = EUNISa_1_descr),
             position = position_jitter(width = 0.15), size = 1, alpha = 0.25) +
  geom_boxplot(width = 0.2, outlier.shape = NA, alpha = 0.5) +
  stat_summary(fun.y=mean, geom="point", shape = 20, size = 1) +
  stat_summary(fun.data = function(x) data.frame(y = max(x) + 0.1,
                                                 label = length(x)),
               geom = "text", aes(label = ..label..), vjust = 0.5) +
  labs(y = "EOS_DOY", x = "EUNISa_1_descr") +
  guides(fill = FALSE, color = FALSE) +
  theme_bw() + coord_flip()
```
...
For doing this, and probably the previous stuff too, we need to ensure that there is only one observation of each plot per year. So far there are some plots with more than one observation per year.
...
So far not including this in models. Do we include plot as a random effect? Nested within unit or not? And year?

Or use interannual means? Then we would have one value per plot - but what about plots where the habitat type changed? (we could tag them as "change").
...
### HERE: ggplot means and error plot for ALP - ALPS with SOS, Peak and EOS together

### HERE: correspondence analyses (CAs), biplot showing the distribution of points, similar to PCA, see if they are grouped by habiat type... (read paper sent by Bea)
