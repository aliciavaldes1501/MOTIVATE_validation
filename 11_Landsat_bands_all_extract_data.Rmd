...
data_RS_Landsat_bands_indices <- data_RS_Landsat_bands_updated %>%
  ...
  #mutate(EVI = if_else(EVI > 1 | EVI < -1, NA, EVI)) # 1220710 values of EVI as NA
```
...
Much fewer days than with S2!

# HERE: Compute phenological metrics from models fitted to time series data
...
execution_time <- system.time({
  with_progress({
    GAM_data <- data_RS_Landsat_bands_indices %>%
...
execution_time <- system.time({
  with_progress({
    smoothed_data <- data_RS_Landsat_bands_indices %>%
      group_split(PlotObservationID) %>%
      set_names(map_chr(., ~ as.character(unique(.x$PlotObservationID)))) %>%
...
ggplot(data_monthly_EUNISa_2 %>% filter(EUNISa_1 == "Q" & !is.na(EUNISa_2)), 
       aes(x = month, y = mean_NDVI, color = EUNISa_2_descr, 
...
# EVI
ggplot(data_monthly_EUNISa_2 %>% filter(EUNISa_1 == "Q" & !is.na(EUNISa_2)), 
       aes(x = month, y = mean_EVI, color = EUNISa_2_descr, 
           group = EUNISa_2_descr)) +
  geom_point(size = 3) +
  geom_line() +
  geom_errorbar(aes(ymin = mean_EVI - sd_EVI, ymax = mean_EVI + sd_EVI), 
...
# SAVI
ggplot(data_monthly_EUNISa_2 %>% filter(EUNISa_1 == "Q" & !is.na(EUNISa_2)), 
       aes(x = month, y = mean_SAVI, color = EUNISa_2_descr, 
           group = EUNISa_2_descr)) +
  geom_point(size = 3) +
  geom_line() +
  geom_errorbar(aes(ymin = mean_SAVI - sd_SAVI, ymax = mean_SAVI + sd_SAVI), 
